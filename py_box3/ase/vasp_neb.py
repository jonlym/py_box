...
#The next keywords pertain to IOPT = 7 (i.e. FIRE)
    ...
    'efirst',     # Created by Jon Lym
    'elast'       # Created by Jon Lym
]
...
int_keys = [
    ...
    'images',     # number of images in a NEB run
    'spring2',    # Added by Jon Lym. Adds spring2 value to INCAR
    'spower',     # Added by Jon Lym. Adds spower value to INCAR
#The next keywords pertain to the VTST add-ons from Graeme Henkelman's group at UT Austin
...
class Vasp(Calculator):
    def __init__(self, restart=None, output_template='vasp', track_output=False, neb=False,
                 **kwargs):
        self.name = 'Vasp'
        self.float_params = {}
        self.exp_params = {}
...
#        print self.atoms_sorted, self.symbol_count
...
#        self.old_incar_params = self.incar_params.copy()
    ...
    def clean(self):
        ...
        files = ['CHG', 'CHGCAR', 'POSCAR', 'INCAR', 'CONTCAR',
                 ...
                 'LOCPOT', 'AECCAR0', 'AECCAR1', 'AECCAR2', 'wavefunction.dat']
        for f in files:
            try:
                os.remove(f)
            except OSError:
    ...
    def read_magnetic_moments(self, atoms):
        ...
        n = 0
        lines = open(self.outcar, 'r').readlines()
        for line in lines:
