...
class GCN(object):
    ...
    Attributes:
        ...
        source_dict - Dict
            ...
            e.g. source_dict = {'Cu': 'Metallic', 'H': 'Empirical'}
    """
        
    def __init__(self, atoms, bulk = None, scale = 1., exceptions = {}, source_dict = None, update = True):
        self.atoms = atoms
        self.bulk = bulk
        self.bulk_CN = {}
        self.CNs = np.zeros(len(atoms))
        self.neighbors = [[] for i in range(len(self.atoms))]
        self._neighbors_direction = {}
        self.periodic_neighbors = [[] for i in range(len(self.atoms))]
    ...
    def _find_direction(self, i):
        ...
        """
        octant = i // len(self.atoms)
        return base10_to_basen(octant, n = 3, width = 3) - np.ones(shape = 3)
